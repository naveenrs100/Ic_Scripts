 ...
 */
public class SonarInstancePermissionsInfo extends Loggable {
	 ...
	 */
	public SonarInstancePermissionsInfo(SonarClient client) {
		this.client = client;
	}
	 ...
	 */
	public SonarGroup group(String name) {
	...
	public void loadPermissionTemplates() {
		...
		templatesObject.permissionTemplates?.each { def template ->
			...
			PERMISSIONS.each { String permissionName ->
	...
	public void saveGroup(SonarGroup g) {
		if (g.doesExistInSonar()) {
			...
			client.post("user_groups/create", [
				"name":g.getName(),
				"description": "Creado en la interfase de usuarios y grupos"
				]);
