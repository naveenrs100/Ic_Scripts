...
Long milis = Stopwatch.watch {
	...
	def permisoClarive = resolver.resolve("permisoClarive");
	...
	if((permisoClarive == null || permisoClarive.trim().equals("true")) && (action.equals("release") || action.equals("addFix") || action.equals("addHotfix"))) {
		...
		if(instantanea == null || instantanea.trim().equals("")) {
			...
			if(resolver.resolve("builtVersion") != null && !resolver.resolve("builtVersion").trim().equals("")) {
				prov_cod_release = resolver.resolve("builtVersion");
				prov_cod_release = resolver.resolve("builtVersion");
			} else {
			...
			if(resolver.resolve("instantanea") != null && !resolver.resolve("instantanea").trim().equals("")) {
				prov_cod_release = resolver.resolve("instantanea");
			} else {
